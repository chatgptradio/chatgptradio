...
def test_scene_cycle_order():
    assert SCENE_CYCLE == ["neural", "chaos", "globe", "network", "spectre"]


def test_scene_cycle_has_five_modes():
    assert len(SCENE_CYCLE) == 5
...
async def test_rotator_advances_from_network():
    ...
    try:
        ...
        pass
    update = await asyncio.wait_for(q.get(), timeout=1.0)
    assert update["visual_mode"] == "spectre"


@pytest.mark.asyncio
async def test_rotator_wraps_around_from_spectre():
    state = GlobalState()
    state.visual_mode = "spectre"
    q: asyncio.Queue = asyncio.Queue()
    task = asyncio.create_task(run_scene_rotator(state, q, interval_s=0))
    await asyncio.sleep(0.05)
    task.cancel()
